...
void getdata(bufferToggle bt) {
  ...
  // works for now would like to find a work around from using this str so i
  // dont
...
int main() {
  ...
  while (true) {
    ...
    // here is where the main menu should be
    cout << "(n)ew\n(l)ist\n(q)uit";
    c = getchar();
    tes[0] = c;
    if (tes[0] == 'n') {
            ...
            if (tes[0] == 'q') {
              ...
              if (tes[0] == 'p') {
                ...
                if (isdigit(tes[0])) {
                  ...
                  // similiar later
                  // am i fucking retarded? i could just use the original int
                  // retard
                  int buff = tes[0] - '0';
                  while (true) {
                    ioctl(STDOUT_FILENO, TIOCGWINSZ, &w);
                    ...
                    if (tes[0] == 'e') {
                      ...
                      while (true) {
                        ...
                        tes[0] = c;
                        buffer[z] = cBuff[0];
                        // should make this a case statement tbh
                        if (c == 127) {
                          ...
                          if (c == 27) {
                            ...
                            if (c == 91) {
                              c = getchar() & 0xFF;
                              if (c == 66 && z < entrySize - w.ws_col) {
                                z += w.ws_col;
                              } else {
                                if (c == 65 && z >= 0 + w.ws_col) {
                                  z -= w.ws_col;
                                } else {
                                  if (c == 68 && z > 0) {
                                    z--;
